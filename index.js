...
// starts the main menu
  ...
  console.log("");
  console.log(
    "Please select one of the followings: [1] Quick Start, [2] Manual Start, [3] Settings"
  );
  prompt.start();
...
function quickStart() {
  // applies default settings and starts the timer
  ...
  quickStartPromise.then((data) => {
    ...
    console.log("");
    console.log(
      `The reminder interval has been set to ${userSettings.interval} minutes.`
    );
...
function manualStart() {
  //asks for user to input desired settings and starts the timer
  prompt.start();
  console.log("");
  console.log(
    "Enter the time interval (in minutes) you want to have between each reminder."
  );
    ...
    function (err, result) {
      ...
      if (userInterval && userInterval > 0 && userInterval <= 60) {
        ...
        function soundPrompt() {
          ...
          console.log(
            "Please select a reminder sound, you can preview these in the settings menu."
          );
          prompt.get(["sound id"], function (err, result) {
            if (allSounds[result["sound id"]]) {
              ...
              setTimeout(() => {
                ...
                console.log("");
                console.log(
                  "Make sure you have your sounds on to hear the reminder!"
                );
                postureTimer(
                  userInterval,
                  `./sounds/${allSounds[result["sound id"]]}.mp3`
                );
                console.time("time");
              }, 1500);
    ...
    switch (result["input"]) {
      case "1":
        ...
        function settingsIntervalPrompt() {
          ...
          prompt.get(
            ...
            function (err, result) {
              if (
                parseInt(result.interval) > 0 &&
                parseInt(result.interval) <= 60
              ) {
                ...
                function settingsSoundPrompt(interval) {
                  ...
                  prompt.get(["sound id"], function (err, result) {
                    if (allSounds[result["sound id"]]) {
                      ...
                      fs.writeFile(
                        "./userSettings.JSON",
                        JSON.stringify(userSettingsObject),
                        () => {
                          console.log("");
                          console.log(
                            `Interval now set to ${interval} minutes.`
                          );
                          console.log(
                            `Sound now set to ${
                              allSounds[result["sound id"]]
                            }.mp3`
                          );
                          console.log("");
      ...
      case "2":
        ...
        function previewSounds() {
          ...
          console.table(allSounds);
          console.log(
            `Enter "exit" if you wish to return to the settings menu.`
          );
...
function postureTimer(maxTime, reminderSound) {
  // the timer, when time is hit, opens the sound file and resets the time
  if (running) {
    if (timer >= maxTime * 60) {
      timer = 0;
      open(reminderSound);
...
function stopPostureTimer() {
  // starts the prompt that asks the user to stop the timer 
  console.log("");
  console.log(
    "If you wish to stop the reminders, type any character and press enter."
  );
  prompt.start();
  prompt.get(
    [{ name: "end", description: "Enter any character to stop" }],
    function (err, result) {
      console.log("");
